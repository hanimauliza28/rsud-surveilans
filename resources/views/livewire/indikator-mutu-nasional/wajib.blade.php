<div>
    <div class="overflow" style="overflow:scroll">
        ...
        <table id="data-table" class="table align-middle table-striped table-row-dashed fs-7 gy-3">
            ...
            <tbody class="text-gray-600 " style="overflow:scroll">
                @foreach ($imut as $indikatorMutu)
                    <tr>
                        <td class="center text-center">{{ $nomor }}</td>
                        <td class="center text-center"><a class="text-success"
                                onclick="detail({{ $indikatorMutu->id }})">{!! $indikatorMutu['judul'] !!}
                            </a>
                            ...
                            <td class="text-center center">
                                ...
                                @php
                                    ...
                                    if ($denumerator == 0) {
                                        ...
                                        if ($indikatorMutu->nama_function == 'waktu-tunggu-rawat-jalan') {
                                            $rata2 = $numerator / $denumerator;
                                            $detik = $rata2 % 60;
                                            $menit = ($rata2 - $detik) / 60;
                                            //$score = floor($menit) . ' min ' . $detik . ' sec';
                                            $score = floor($menit) . ' min';

                                            $numerator_sisa = $numerator % 60;
                                            $numerator = ($numerator - $numerator_sisa)/60;
                                            $hasilSurvey->numerator = $numerator;
                                        } else {
                                ...
                                {{-- conditional indikator mutu --}}

                                @if ($indikatorMutu->nama_function == 'waktu-tunggu-rawat-jalan')
                                    <a onclick="masukanNilai({{ $indikatorMutu->id . ',' . $x . ',' . $month . ',' . $year }})"
                                        class="badge text-hover-white badge-success mb-2 hoverable">{{ $score ?? 0 }}
                                    </a>
                                @else
                                    <a onclick="masukanNilai({{ $indikatorMutu->id . ',' . $x . ',' . $month . ',' . $year }})"
                                ...
                                <div class="d-flex justify-content-center">
                                    <span
                                        class="badge bg-light-success text-gray-500 me-1 center">{{ $hasilSurvey->numerator ?? 0 }}</span>
                                    <span
                                        class="badge bg-light-success text-gray-500 center">{{ $hasilSurvey->denumerator ?? 0 }}</span>
                                </div>
                                {{-- <a onclick="detailNilai({{ $indikatorMutu->id }})"
                                    class="badge bg-hover-success text-hover-white badge-light-success mt-2">Detail</a> --}}

                                <a onclick="sync({{ $indikatorMutu->id }}, '{{ $tglKolom }}')"
